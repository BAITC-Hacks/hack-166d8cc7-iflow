...
from app.services.progress_engine import historical_completions, project_skills, participation_views
...
def employee(employee_id: str,ctx: RequestContext=Depends(employee_read_context)):
    ...
    history=ctx.snapshot.history.for_employee(employee_id)
    return EmployeeDetail(profile=profile,current_skills=project_skills(profile,historical_completions(history),ctx.snapshot.events,ctx.as_of_date),
                          history=participation_views(history),revision=ctx.snapshot.revision,as_of_date=ctx.as_of_date)
