...
import { ArrowRight, ArrowUpRight, Bell, BookOpen, CalendarDays, Check, CheckCheck, ChevronRight, CircleHelp, Clock3, Flag, Gift, LayoutDashboard, LogOut, Map as MapIcon, Menu, RefreshCw, Search, ShieldCheck, ShoppingBag, Sparkles, Trophy, Users, X } from 'lucide-react';
...
import { actualGain, categories, completionOptions, displayDate, eventArt, eventColor, formats, hours, initials, reasonNames, skillName, statusNames } from '@/lib/career-data';
import { Art, Scene } from './artwork';
...
function Workspace({ session, initialPage, requestedEmployee, onLogout }: { session: Session; initialPage: Page; requestedEmployee?: string; onLogout: () => void }) {
  ...
  useEffect(() => {
    ...
    setLoading(true); setLoadError(''); setAI(null); setAIStatus(''); aiController.current?.abort(); setAIBusy(false);
  ...
  async function askAI() {
    if (!context || aiBusy || loading) return;
    aiController.current?.abort(); const controller = new AbortController(); aiController.current = controller;
    const original = context;
    setAIBusy(true); setAIStatus(''); setAI(null);
    try {
      const result = await requestRecommendations(employeeId, token, controller.signal);
      if (!controller.signal.aborted && active.current && currentContext.current?.employee_id === result.employee_id && currentContext.current?.revision === result.revision && currentContext.current?.as_of_date === result.as_of_date) setAI(result);
      else if (!controller.signal.aborted && active.current) setAIStatus('Данные изменились. Обнови профиль и запроси рекомендации снова.');
    } catch (error) {
      if (!controller.signal.aborted && active.current && currentEmployee.current === original.employee_id) setAIStatus(error instanceof ApiError && error.status === 501 ? 'Провайдер ИИ ещё не подключён на сервере. Карта продолжает показывать доступные шаги по правилам каталога.' : message(error));
    } finally { if (active.current && !controller.signal.aborted) setAIBusy(false); }
  }
  ...
  return <div className="app-shell connected-shell">
    ...
    <aside className={`sidebar ${menu ? 'open' : ''}`}><button className="brand" onClick={() => navigate('roadmap')} aria-label="Halyk Career Quest — главная"><img src="/halyk-logo.png" alt="Halyk"/><span>CAREER QUEST <span>↗</span></span></button><div className="workspace-label">ТВОЯ ТЕРРИТОРИЯ РОСТА</div><nav aria-label="Основная навигация">{navigation.map(item => <button key={item.id} className={`nav-item ${page === item.id ? 'active' : ''}`} onClick={() => navigate(item.id)} aria-current={page === item.id ? 'page' : undefined}><item.icon size={19}/><span>{labels[item.id]}</span>{item.id === 'events' && context && <span className="nav-count">{context.event_catalog.length}</span>}</button>)}</nav><div className="sidebar-promo"><div className="promo-icon"><Flag size={21}/><span className="tiny-star">✦</span></div><h3>Расти в своём ритме</h3><p>Каждый небольшой шаг<br/>ведёт к большим переменам.</p><button onClick={() => navigate('roadmap')}>Найти свой шаг<ArrowUpRight size={16}/></button></div><div className="sidebar-bottom">{isHR && <button className={`nav-item ${page === 'hr' ? 'active' : ''}`} onClick={() => navigate('hr')}><Users size={19}/><span>HR-панель</span></button>}<button className="nav-item" onClick={() => setHelp(true)}><CircleHelp size={19}/><span>Как это работает</span></button><button className="nav-item" onClick={onLogout}><LogOut size={18}/><span>Сменить профиль</span></button><button className="sidebar-profile" onClick={() => navigate(isHR ? 'hr' : 'home')}><span className="avatar">{identityName}</span><span><b>{name}</b><small>{isHR ? 'Доступ HR' : profile?.role ?? 'Сотрудник'}</small></span></button></div></aside>
    <div className="main-shell"><header className="topbar"><div className="breadcrumb"><button className="icon-button mobile-menu" aria-label="Открыть меню" onClick={() => setMenu(true)}><Menu size={22}/></button><span>Моё развитие</span><ChevronRight size={14}/><strong>{labels[page]}</strong></div><div className="header-actions"><span className="demo-status"><span/>Данные сервера</span>{market?.balance != null && <button className="balance-pill" onClick={() => navigate('market')} aria-label={`Баланс ${market.balance} монет`}><Coin/><b>{market.balance}</b></button>}<button className="icon-button" aria-label="Обновить данные" disabled={loading || mutating} onClick={refresh}><RefreshCw size={18}/></button><button className="icon-button bell" aria-label="Мои назначения" onClick={() => { setFilter('Мои назначения'); navigate('events'); }}><Bell size={20}/></button><button className="avatar header-avatar" aria-label="Открыть профиль" onClick={() => navigate(isHR ? 'hr' : 'home')}>{identityName}</button></div></header>
    <main className="main-content" id="main-content">
      {!isHR && (page === 'home' || page === 'roadmap') && <EmployeeNotificationCenter token={token} employeeId={employeeId}/>}
      {notice && <div className="journey-celebration" role="status"><span className="celebration-star">✦</span><div><b>Ещё один шаг вперёд</b><p>{notice}</p></div><button className="icon-button" aria-label="Закрыть уведомление" onClick={() => setNotice('')}><X size={17}/></button></div>}
